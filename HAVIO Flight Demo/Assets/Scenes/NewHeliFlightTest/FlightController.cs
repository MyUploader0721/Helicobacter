...
public class FlightController : MonoBehaviour
{
    ...
    float fCollectiveSpeed = 5.0f;
    float fAntiTorqueSpeed = 1.5f;
    float fCycleSpeed      = 2.0f;
    ...
    {
        objCollectiveLever.transform.rotation = Quaternion.Euler(-inputController.fCollective * 10.0f, 0.0f, 0.0f);

        Vector3 v3Cycle = new Vector3(inputController.fCyclePitch, 0.0f, -inputController.fCycleRoll);

        for (int i = 0; i < objCycleControllers.Length; i++)
        {
            objCycleControllers[i].transform.eulerAngles = v3Cycle * 10.0f;
        }
    }
}
